...
public class ProducteurDao extends Dao implements IDao<Producteur> {
    ...
    @Override
    public boolean addMany(ArrayList<Producteur> entities) {
        for (Producteur entity : entities) {
            add(entity);
        }
        return true;
    }
    ...
    private Arbre appendDataToArbre(Arbre arbre, Cursor cursor) {
        ...
        arbre.setIdProducteur(cursor.getLong(35));
        arbre.setCodeProducteur(cursor.getString(36));
        return arbre;
    }
